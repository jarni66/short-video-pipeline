...
if not config.app.get("hide_config", False):
    with st.expander(tr("Basic Settings"), expanded=False):
        ...
        with right_config_panel:
            ...
            youtube_api_key = get_keys_from_config("youtube_api_keys")
            youtube_api_key = st.text_input(
                tr("YouTube Data API Key"), value=youtube_api_key, type="password"
            )
            save_keys_to_config("youtube_api_keys", youtube_api_key)
...
if start_button:
    ...
    if params.video_source == "youtube" and not config.app.get("youtube_api_keys", ""):
        st.error(tr("Please Enter the YouTube Data API Key"))
        scroll_to_bottom()
        st.stop()
